...
class Sam2PatchEmbeddings(nn.Cell):
    r"""
...
class Sam2SinePositionEmbedding(nn.Cell):
    """
    ...
    def construct(
        ...
        if mask is None:
            mask = mint.zeros((x.shape[0], x.shape[2], x.shape[3]), dtype=ms.bool)
...
class Sam2VisionNeck(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> Tuple[Tuple[ms.Tensor, ...], Tuple[ms.Tensor, ...]]:
        fpn_hidden_states = ()
...
def do_pool(x: ms.Tensor, query_stride: Optional[int] = None) -> ms.Tensor:
...
class Sam2MultiScaleAttention(nn.Cell):
    def __init__(
        self,
        ...
        self.qkv = mint.nn.Linear(dim, dim_out * 3)
        self.proj = mint.nn.Linear(dim_out, dim_out)
    ...
    def construct(self, hidden_states: ms.Tensor, **kwargs) -> ms.Tensor:
        ...
        query, key, value = mint.unbind(qkv, 2)

        attn_weights = (query * self.scale) @ key.transpose(-2, -1)
        attn_weights = mint.softmax(attn_weights, dtype=ms.float32, dim=-1).to(query.dtype)
...
class Sam2MultiScaleBlock(GradientCheckpointingLayer):
    def __init__(
        ...
        self.layer_norm2 = mint.nn.LayerNorm(self.dim_out, eps=config.layer_norm_eps)
        ...
        if self.dim != self.dim_out:
            self.proj = mint.nn.Linear(self.dim, self.dim_out)
...
class Sam2HieraDetModelOutput(ModelOutput):
    ...
    intermediate_hidden_states (`Tuple[ms.Tensor]` of shape `(batch_size, height, width, hidden_size)`):
...
class Sam2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (mint.nn.LayerNorm, Sam2LayerNorm)):
...
class Sam2HieraDetModel(Sam2PreTrainedModel, GenerationMixin):
    ...
    def _get_pos_embed(self, hw: Tuple[int, int]) -> ms.Tensor:
        ...
        pos_embed = pos_embed + window_embed.tile(tuple([x // y for x, y in zip(pos_embed.shape, window_embed.shape)]))
    ...
    def construct(
        ...
        for i, block_module in enumerate(self.blocks):
            hidden_states = block_module(hidden_states)
...
class Sam2VisionModel(Sam2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Sam2VisionConfig):
        ...
        self.backbone = Sam2HieraDetModel(config.backbone_config)
...
class Sam2PositionalEmbedding(nn.Cell):
...
class Sam2MaskEmbedding(nn.Cell):
    def __init__(self, config: Sam2PromptEncoderConfig):
        super().__init__()
...
class Sam2PromptEncoder(nn.Cell):
    def __init__(self, config: Sam2PromptEncoderConfig):
        super().__init__()
...
class Sam2TwoWayAttentionBlock(nn.Cell):
...
class Sam2TwoWayTransformer(nn.Cell):
...
class Sam2LayerNorm(mint.nn.LayerNorm):
...
class Sam2MaskDecoder(nn.Cell):
    ...
    def construct(
        ...
        if sparse_prompt_embeddings.shape[0] != 0:
            sparse_prompt_embeddings = sparse_prompt_embeddings.to(output_tokens.dtype)
