...
class Sam2VideoProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        input_points: Optional[Union[list[list[list[list[float]]]], ms.Tensor]] = None,
        input_labels: Optional[Union[list[list[list[int]]], ms.Tensor]] = None,
        input_boxes: Optional[Union[list[list[list[float]]], ms.Tensor]] = None,
        original_sizes: Optional[Union[list[list[float]], ms.Tensor]] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            input_points (`list[list[list[list[float]]]]`, `ms.Tensor`, *optional*):
                The points to add to the frame.
            input_labels (`list[list[list[int]]]`, `ms.Tensor`, *optional*):
                The labels for the points.
            input_boxes (`list[list[list[float]]]`, `ms.Tensor`, *optional*):
                The bounding boxes to add to the frame.
            original_sizes (`list[list[float]]`, `ms.Tensor`, *optional*):
        ...
        Returns:
            ...
            - `pixel_values` (`ms.Tensor`): The processed image(s).
            - `original_sizes` (`list[list[float]]`): The original sizes of the images.
            - `reshaped_input_sizes` (`ms.Tensor`): The reshaped input sizes of the images.
            - `labels` (`ms.Tensor`): The processed segmentation maps (if provided).
            - `input_points` (`ms.Tensor`): The processed points.
            - `input_labels` (`ms.Tensor`): The processed labels.
            - `input_boxes` (`ms.Tensor`): The processed bounding boxes.
        """
        ...
        if input_points is not None or input_labels is not None or input_boxes is not None:
            ...
            if processed_points is not None:
                ...
                final_points = ms.tensor(padded_points, dtype=ms.float32)
            ...
            if processed_labels is not None:
                ...
                final_labels = ms.tensor(padded_labels, dtype=ms.int64)
    ...
    def _normalize_coordinates(
        self, target_size: int, coords: "ms.Tensor", original_size, is_bounding_box=False
    ) -> "ms.Tensor":
        """
    ...
    def _convert_to_nested_list(self, data, expected_depth, current_depth=0):
        ...
        if isinstance(data, ms.Tensor):  # Mindspore tensor
    ...
    def _get_nesting_level(self, input_list):
        ...
        elif isinstance(input_list, (np.ndarray, ms.Tensor)):
    ...
    def _validate_single_input(
        self,
        data: Union[ms.Tensor, np.ndarray, list],
        expected_depth: int,
        input_name: str,
        expected_format: str,
        ...
        if isinstance(data, (ms.Tensor, np.ndarray)):
    ...
    def post_process_masks(
        ...
        **kwargs,
        ...
        Args:
            masks (`Union[List[ms.Tensor], List[np.ndarray]]`):
                Batched masks from the mask_decoder in (batch_size, num_channels, height, width) format.
            original_sizes (`Union[ms.Tensor, List[Tuple[int,int]]]`):
        ...
        Returns:
            (`ms.Tensor`): Batched masks in batch_size, num_channels, height, width) format, where (height, width)
    ...
    def init_video_session(
        ...
        Args:
            ...
            dtype (`ms.dtype`, *optional*, defaults to `ms.float32`):
                The mindspore dtype to use for the whole session.
        """
    ...
    def add_inputs_to_inference_session(
        ...
        input_labels: Optional[Union[list[list[list[int]]], ms.Tensor]] = None,
        input_boxes: Optional[Union[list[list[list[float]]], ms.Tensor]] = None,
        input_masks: Optional[Union[np.ndarray, ms.Tensor, list[np.ndarray], list[ms.Tensor]]] = None,
        ...
        Args:
            ...
            input_boxes (`list[list[list[float]]]`, `ms.Tensor`, *optional*):
                The bounding boxes to add to the frame.
            input_masks (`np.ndarray`, `ms.Tensor`, `list[np.ndarray]`, or `list[ms.Tensor]`, *optional*):
    ...
    def process_new_points_or_boxes_for_video_frame(
        ...
        if input_points is not None:
            ...
            input_points = mint.zeros(1, len(obj_ids), 0, 2, dtype=ms.float32)
        if input_labels is not None:
            ...
            input_labels = mint.zeros(1, len(obj_ids), 0, dtype=ms.int32)
        ...
        if input_boxes is not None:
            ...
            box_labels = ms.tensor([2, 3], dtype=ms.int32).repeat(1, box_coords.shape[1], 1)
            input_points = mint.cat([box_coords, input_points], dim=2)
            input_labels = mint.cat([box_labels, input_labels], dim=2)
    ...
    def process_new_mask_for_video_frame(
        ...
        frame_idx: int,
        obj_ids: list[int],
        input_masks: Union[np.ndarray, ms.Tensor, list[np.ndarray], list[ms.Tensor]],
    ):
        """
        ...
        Args:
            ...
            obj_ids (`list[int]`):
                ...
                These can be any integers and can be reused later on to specify an object.
            input_masks (`np.ndarray`, `ms.Tensor`, `list[np.ndarray]`, or `list[ms.Tensor]`):
                The mask(s) to add to the frame.
        """
        ...
        for obj_id, mask in zip(obj_ids, input_masks):
            ...
            if not isinstance(mask, ms.Tensor):
                mask = ms.tensor(mask, dtype=ms.bool)
            nb_dim = mask.dim()
            ...
            if mask_H != self.target_size or mask_W != self.target_size:
                mask_inputs = mint.nn.functional.interpolate(
                    mask_inputs_orig,
