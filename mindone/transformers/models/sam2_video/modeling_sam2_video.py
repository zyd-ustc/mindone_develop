...
class Sam2VideoInferenceSession:
    ...
    def __init__(
        ...
        self.processed_frames = (
            dict(enumerate(video.to(dtype=dtype))) if video is not None else None
        )
    ...
    def add_new_frame(self, pixel_values: ms.Tensor, frame_idx: Optional[int] = None) -> int:
        ...
        pixel_values = pixel_values.to(dtype=self.dtype, non_blocking=True)
    ...
    def get_frame(self, frame_idx: int) -> ms.Tensor:
        ...
        return self.processed_frames[frame_idx]
...
class Sam2VideoPositionEmbeddingSine(nn.Cell):
    """
...
class Sam2VideoAttention(nn.Cell):
    """
...
class Sam2VideoTwoWayAttentionBlock(nn.Cell):
...
class Sam2VideoFeedForward(nn.Cell):
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int,
        ...
        self.layers = nn.CellList([mint.nn.Linear(hidden_dim, hidden_dim) for _ in range(num_layers - 2)])
...
class Sam2VideoVisionRotaryEmbedding(nn.Cell):
    ...
    def __init__(self, config: Sam2VideoConfig):
        ...
        flattened_indices = mint.arange(end_x * end_y, dtype=ms.long)
...
def apply_rotary_pos_emb_2d(
    ...
    if k_rot.shape[-2] == 0:
        ...
        return q_embed.type_as(q), mint.cat([k_rot, k_pass], dim=-2)
    ...
    k_embed = mint.cat([k_embed.type_as(k), k_pass], dim=-2)
    return q_embed.type_as(q), k_embed


class Sam2VideoRoPEAttention(nn.Cell):
...
class Sam2VideoMemoryAttentionLayer(nn.Cell):
    def __init__(self, config: Sam2VideoConfig):
        super().__init__()
        ...
        self.linear1 = mint.nn.Linear(hidden_size, config.memory_attention_feed_forward_hidden_size)
        self.dropout = mint.nn.Dropout(config.memory_attention_dropout)
        self.linear2 = mint.nn.Linear(config.memory_attention_feed_forward_hidden_size, hidden_size)
...
class Sam2VideoMemoryAttention(nn.Cell):
    def __init__(self, config: Sam2VideoConfig):
        super().__init__()
...
class Sam2VideoMemoryFuser(nn.Cell):
    def __init__(self, config: Sam2VideoConfig):
        super().__init__()
        self.layers = nn.CellList(
...
class Sam2VideoMaskDownSamplerLayer(nn.Cell):
...
class Sam2VideoMaskDownSampler(nn.Cell):
    """
...
class Sam2VideoMemoryEncoder(nn.Cell):
    def __init__(self, config: Sam2VideoConfig):
        super().__init__()
...
class Sam2VideoPositionalEmbedding(nn.Cell):
...
class Sam2VideoMaskEmbedding(nn.Cell):
    def __init__(self, config: Sam2VideoPromptEncoderConfig):
        super().__init__()
...
class Sam2VideoPromptEncoder(nn.Cell):
    def __init__(self, config: Sam2VideoPromptEncoderConfig):
        super().__init__()
    ...
    def _embed_points(self, points: ms.Tensor, labels: ms.Tensor, pad: bool) -> ms.Tensor:
        ...
        # mint.where and expanding the labels tensor is required by the ONNX export
...
class Sam2VideoTwoWayTransformer(nn.Cell):
...
class Sam2VideoMaskDecoder(nn.Cell):
    def __init__(self, config: Sam2VideoMaskDecoderConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def _single_frame_forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if multimask_output:
            ...
            best_iou_inds = mint.argmax(iou_scores, dim=-1)  # shape: (batch_size, num_objects)
            batch_inds = mint.arange(batch_size)
            object_batch_inds = mint.arange(num_objects)
            # Create meshgrid for proper broadcasting in MindSpore
            # PyTorch automatically broadcasts, but MindSpore needs explicit handling
            batch_grid, obj_grid = mint.meshgrid(batch_inds, object_batch_inds, indexing='ij')
            batch_flat = batch_grid.flatten()
            obj_flat = obj_grid.flatten()
            best_flat = best_iou_inds.flatten()
            # Use flattened indices to select masks
            low_res_masks_flat = low_res_multimasks[batch_flat, obj_flat, best_flat]
            high_res_masks_flat = high_res_multimasks[batch_flat, obj_flat, best_flat]
            # Reshape back to (batch_size, num_objects, H, W)
            mask_shape = low_res_masks_flat.shape[1:]
            low_res_masks = low_res_masks_flat.view(batch_size, num_objects, *mask_shape)
            ...
            if sam_output_tokens.shape[2] > 1:
                ...
                token_shape = sam_output_token_flat.shape[1:]
                sam_output_token = sam_output_token_flat.view(batch_size, num_objects, *token_shape)
        else:
